...
RSpec.describe(Hanami::CLIBulma::Commands::App::Generate::View, :app) do
  subject { described_class.new(fs: fs, inflector: inflector, generator: generator) }

  let(:out) { StringIO.new }
  let(:fs) { Hanami::CLI::Files.new(memory: true, out: out) }
  let(:inflector) { Dry::Inflector.new }
  let(:generator) { Hanami::CLIBulma::Generators::App::View.new(fs: fs, inflector: inflector) }
  ...
  context "generating for a slice" do
    it "generates a view in a top-level namespace" do
      within_application_directory do
        ...
        template_file = <<~EXPECTED
          <h1>Main::Views::Users::Index</h1>\n\n\n<p>This is supposed to work with bulma</p>
        EXPECTED
